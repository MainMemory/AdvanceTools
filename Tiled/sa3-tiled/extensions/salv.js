var salvMapFormat = {
	...
	read: function(fileName) {
		...
		if (stginf.Background1 != null) {
			...
			background1 = JSON.parse(txtfile.readAll());
			txtfile.close();
			if (fileName.endsWith("Map.salv"))
				readPalette(background1, palette);
		}

		var background2 = null;
		if (stginf.Background2 != null) {
			txtfile = new TextFile(FileInfo.joinPaths(projpath, FileInfo.fromNativeSeparators(stginf.Background2)), TextFile.ReadOnly);
			background2 = JSON.parse(txtfile.readAll());
			txtfile.close();
			if (fileName.endsWith("Map.salv"))
				readPalette(background2, palette);
		}
		...
		if (background1 != null)
			tilemap.addLayer(readBackgroundLayer("Background 1", background1, palette));
		if (background2 != null)
			tilemap.addLayer(readBackgroundLayer("Background 2", background2, palette));
		if (foregroundLow != null)
