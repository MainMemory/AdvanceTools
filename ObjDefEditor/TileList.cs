...
namespace ObjDefEditor
{
	...
	public partial class TileList : UserControl
	{
		...
		private void TileList_Paint(object sender, PaintEventArgs e)
		{
			...
			double ratio = imageWidth / (double)imageHeight;
			switch (Direction)
			{
				case Direction.Horizontal:
					...
					for (int c = stc; c < edc; c++)
						for (int r = 0; r < tilesPerCol; r++)
						{
							...
							var img = Images[i];
							int w, h;
							if (img.Width / (double)img.Height < ratio)
							{
								h = imageHeight;
								w = (int)(img.Width * (imageHeight / (double)img.Height));
							}
							else
							{
								w = imageWidth;
								h = (int)(img.Height * (imageWidth / (double)img.Width));
							}
							g.DrawImage(img, (actualImageWidth * c) + 2 - hScrollBar1.Value + (imageWidth - w) / 2, (actualImageHeight * r) + 2 + (imageHeight - h) / 2, w, h);
							i++;
				...
				case Direction.Vertical:
					...
					for (int r = str; r < edr; r++)
						for (int c = 0; c < tilesPerRow; c++)
						{
							...
							g.DrawImage(img, (actualImageWidth * c) + 2 + (imageWidth - w) / 2, (actualImageHeight * r) + 2 - vScrollBar1.Value + (imageHeight - h) / 2, w, h);
							i++;
							if (i == Images.Count) return;
						}
					break;
