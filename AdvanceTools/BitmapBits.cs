...
namespace AdvanceTools
{
	...
	public class BitmapBits
	{
		...
		public static BitmapBits FromTile4bpp(byte[] art, int index)
		{
			BitmapBits bmp = new BitmapBits(8, 8);
			if (index + 32 <= art.Length)
			{
				for (int i = 0; i < 32; i++)
				{
					bmp.Bits[i * 2] = (byte)(art[i + index] & 0xF);
					bmp.Bits[i * 2 + 1] = (byte)(art[i + index] >> 4);
				}
			}
			return bmp;
		}

		public byte[] ToTile4bpp(int x, int y)
		{
			List<byte> res = new List<byte>();
			for (int _y = 0; _y < 8; _y++)
				for (int _x = 0; _x < 8; _x += 2)
					res.Add((byte)((this[x + _x, y + _y] & 0xF) | ((this[x + _x + 1, y + _y] & 0xF) << 4)));
			return res.ToArray();
		}

		public byte[] ToTile4bpp(Point pnt) => ToTile4bpp(pnt.X, pnt.Y);

		public byte[] ToTile4bpp() => ToTile4bpp(0, 0);

		public static BitmapBits FromTile8bpp(byte[] art, int index)
		{
			BitmapBits bmp = new BitmapBits(8, 8);
			Array.Copy(art, index, bmp.Bits, 0, bmp.Bits.Length);
			return bmp;
		}

		public byte[] ToTile8bpp(int x, int y)
		{
			return GetSection(x, y, 8, 8).Bits;
		}

		public byte[] ToTile8bpp(Point pnt) => ToTile8bpp(pnt.X, pnt.Y);

		public byte[] ToTile8bpp() => ToTile8bpp(0, 0);
