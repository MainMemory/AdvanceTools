	...
	static class Program
	{
		...
		static void CurrentDomain_UnhandledException(object sender, UnhandledExceptionEventArgs e)
		{
			if (MainForm.Instance != null)
			{
				...
				System.IO.File.WriteAllLines("AdvanceBG.log", MainForm.Instance.LogFile.ToArray());
				using (ErrorDialog ed = new ErrorDialog("Unhandled Exception " + e.ExceptionObject.GetType().Name + "\nLog file has been saved.", false))
					ed.ShowDialog();
			}
			else
			{
				System.IO.File.WriteAllText("AdvanceBG.log", e.ExceptionObject.ToString());
				MessageBox.Show("Unhandled Exception " + e.ExceptionObject.GetType().Name + "\nLog file has been saved.", "AdvanceBG Fatal Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
		}
	}
}
